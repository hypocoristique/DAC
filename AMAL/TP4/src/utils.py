...
class SampleMetroDataset(Dataset):
    def __init__(self, data,length=20,stations_max=None):
        ...
        if stations_max is None:
            ...
            #Station max permet de dire la normalisation qu'on fait. A None pendant le train
            # Pour le test, on peut passer le station_max du train pour avoir la même normalisation de par et d'autre
        else :
